...
const QUICKSTART_HEADERS = `Base URL: https://api.example.com
...
const ROLE_GUIDES = [
  {
    ...
    example: `curl --request POST 'https://api.example.com/api/user/login' \\
  ...
  },
  {
    ...
    example: `curl --request POST 'https://api.example.com/api/admin/login' \\
  --header 'Content-Type: application/json' \\
  ...
  },
  {
    ...
    example: `curl --request POST 'https://api.example.com/api/platform/login' \\
  --header 'Content-Type: application/json' \\
  ...
  },
  {
    id: "role-owner",
    title: "买断总控端（Level 1）",
    scope: "平台所有者总控；与代理共用 /api/platform 登录，但 Level 1 才能进入 operators 等总控范围。",
    fields: "platform_key、account、password；真实平台 KEY 不出现在网页、日志或示例中。",
    success: "返回有时效性的 access_token、Level 1 身份和总控权限；买断总控端没有 Refresh Token。",
...
const SYSTEMS: SystemGuide[] = [
  ...
  },
  {
    ...
    flow: ["读取 policy", "POST snapshot 或 upload", "GET 详情确认 snapshot_id", "POST restore（不提交确认字段）", "恢复后读取业务数据核验"],
    endpoints: [
...
export const metadata = {
  title: "接口文档",
  description: "易云盈四角色与功能系统接入文档，包含真实路由、关键字段、成功失败语义和最小闭环。",
};
  ...
  return (
    <main className="docs-page">
      ...
      <section className="docs-hero">
        <div><p>PUBLIC INTEGRATION GUIDE</p><h1>四角色、十二大业务系统 + 内嵌治理，按闭环接入</h1><span>只列已存在路由的客户接入说明：功能、用途、联动、前置条件、关键参数、成功与失败以及最小使用流程。不提供在线执行，也不展示真实 KEY、密码或 Token。</span></div>
        <aside><ShieldCheck aria-hidden="true" /><strong>安全占位示例</strong><span>HTTPS · 租户隔离 · 时效 Token</span></aside>
      </section>
      ...
      <div className="docs-layout">
        ...
        <div className="docs-content">
          <section className="docs-intro" id="quickstart">
            ...
            <p>以下片段沿用前文 Content-Type，并在需要时注明 Bearer；生产 Base URL 由开发者写入源码，最终用户不填写服务器地址。<code>app_key</code> 与 <code>X-App-Key</code> 表示同一个“应用 API 唯一标识”，不是 <code>app_secret</code>；服务端仍用它绑定租户。管理员登录同时提交 <code>platform_key + app_key</code>，代理与买断总控共用平台登录并由 Level 2/1 区分。</p>
            <div className="docs-code-block" aria-label="接入请求头占位示例">
              <div><span>Base URL</span><code>https://api.example.com</code></div>
            ...
            <div className="docs-warning"><AlertTriangle aria-hidden="true" /><p><strong>示例值全部是明显占位符。</strong>不要把真实密码、平台 KEY 或 Token 放入网页、工单、截图或日志。下载 APK 也不会授予角色权限，实际权限始终由后端账号、层级、应用、Token 和状态共同校验。</p></div>
          </section>
          ...
          <section className="upload-section" id="uploads">
            ...
            <ul><li><Check aria-hidden="true" />客户端预检扩展名、MIME 和体积，服务端必须重新验证实际内容。</li><li><Check aria-hidden="true" />文件名和存储路径由服务端生成，不拼接用户输入。</li><li><Check aria-hidden="true" />论坛、聊天、群文件和商城资源只引用已完成安全检查的 upload_id/资源。</li><li><Check aria-hidden="true" />下载使用受控或短时地址，不暴露真实存储路径。</li></ul>
          </section>
