...
public class LoginRoleUiTest {
    ...
    @Test
    public void editionLocksRoleAndShowsOnlyRequiredTenantField() {
        try (ActivityScenario<LoginActivity> ignored = ActivityScenario.launch(LoginActivity.class)) {
            ...
            if (BuildConfig.FIXED_ROLE.equals("user")) {
                Espresso.onView(withId(R.id.appKeyLayout)).check(matches(isDisplayed()));
                ...
                Espresso.onView(withId(R.id.platformKeyLayout)).check(matches(isDisplayed()));
