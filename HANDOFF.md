...
>
> 状态时间：2026-08-11（Asia/Shanghai）。本地 2.7.15/code60 已完成四端 Debug Build；当前尚未 Finalize、部署、推送或归档，线上仍是 2.7.14/code59。

## 0. 一句话结论

本地 `2.7.15 (60)` 已从源码提交 A 完成四端 Debug Build、全量测试、Lint、identity 与 APK 审计。旧 CRLF 资产已撤销隔离，本次 exact Git blob/worktree identity 字节一致且重新 Build 通过。本次待提交的构建元数据与文档将构成证据提交 B；精确 B 不预写，流程固定为 B → annotated tag → Finalize，权威状态以 finalized manifest 与 Git refs 回读为准。当前尚未 Finalize、生产迁移/部署、推送或归档；线上仍为 `2.7.14 (59)`，且链路仍是 Debug/HTTP，不是正式商业 Release。
...
线上 2.7.14 的最终部署证据由注释标签 `v2.7.14-debug` 固定。2.7.15 Build 源码提交 A 为 `3e744a16c02aae5987ad433d53da28fc4703f93c`；本次待提交内容构成证据提交 B，精确 B 以 Finalize 后 `release-manifest.releaseEvidenceCommit` 为权威，不在自指文档中猜测。目标 annotated tag 只有 Finalize 核验后有效。接手时先运行 `git status --short --branch`，不要从旧归档反向覆盖规范源码。
...
- Build 源码提交 A：`3e744a16c02aae5987ad433d53da28fc4703f93c`。
- schema 4 pending manifest SHA-256：`F465B8AE2CF4D9F0CC7CD21C2DCDE9F0E6E1B0B323C2A18E2468E5C59E34FE15`；release identity SHA-256：`9AC4EFDF7AC8DF8743D7D32ACFBD98A5723D687CC8B2E33BE2B7C8A4ECBFAC9C`。
- 旧 CRLF 资产已撤销隔离；exact Git blob/worktree identity 字节一致并据此重新 Build PASS。

| 端 | 文件 | 字节 | SHA-256 |
| --- | --- | ---: | --- |
| 用户端 | `yiyunying-user-v2.7.15-debug.apk` | 96,707,619 | `9BBA2824B51E9F23EEE1026A74C64E43BB7A98AE2019106831FF9E2B28BAD89A` |
...
> 易运盈本地 2.7.15（60）已从源码提交 A `3e744a16c02aae5987ad433d53da28fc4703f93c` 完成 exact identity 四端 Debug 重建：旧 CRLF 资产撤销隔离，Git blob/worktree 字节一致，226/226 tasks、四端各 337 项/合计 1,348 项测试、Lint 和独立 APK 审计全部通过。pending manifest SHA 为 `F465B8AE2CF4D9F0CC7CD21C2DCDE9F0E6E1B0B323C2A18E2468E5C59E34FE15`，identity SHA 为 `9AC4EFDF7AC8DF8743D7D32ACFBD98A5723D687CC8B2E33BE2B7C8A4ECBFAC9C`。本次待提交内容构成 B，随后 annotated tag → Finalize；精确 B 只按 finalized manifest 回读。当前尚未 Finalize、迁移/部署、推送或归档，线上仍为 2.7.14（59），Debug/HTTP 非正式商业 Release。
