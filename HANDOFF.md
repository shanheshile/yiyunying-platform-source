...
>
> 状态时间：2026-08-11（Asia/Shanghai）。本地 2.7.15/code60 已完成四端 Debug Build；本次待提交的构建元数据/文档将构成证据提交 B，当前尚未 Finalize、部署、推送或归档；线上仍是 2.7.14/code59。

## 0. 一句话结论

本地 `2.7.15 (60)` 已从源码提交 A 完成四端 Debug Build、全量测试、Lint 和 pending manifest 校验。本次待提交的构建元数据与文档将构成证据提交 B；精确 B 哈希不预写，由 Finalize 后 `release-manifest.releaseEvidenceCommit` 确认。目标 annotated tag 只有 Finalize 核验后才算有效发布证据，实际状态以 finalized manifest 与 Git refs 回读为准。当前尚未 Finalize、执行生产迁移/部署、推送或归档；线上仍为 `2.7.14 (59)`。当前链路使用 Debug 签名和 HTTP，不能称为正式商业 Release。
...
线上 2.7.14 的最终部署证据由注释标签 `v2.7.14-debug` 固定，精确提交、归档目录和外层 ZIP SHA-256 以 E 盘归档旁的 `RELEASE_STATE.md` 与顶层 manifest 为准。2.7.15 Build 源码提交 A 为 `49f44235af61b02bba7b6c06f5fe084685f1f2fb`；本次待提交内容构成证据提交 B，精确 B 以 finalized manifest 为准，不在自指文档中猜测。目标 annotated tag 只有 Finalize 核验后有效，生产部署和归档状态另行回读。接手时先运行 `git status --short --branch`，不要从旧归档反向覆盖规范源码。

## 2. 我们在做什么

本轮正在把管理端四栏、多应用控制、BuildConfig 登录身份链、用户快捷模块、三态审核、短视频、相机预览/变焦/录像聚焦、论坛评论线程与排序分页、群管理、资源商店私有审核以及全局主题/弹层修复整理为 `2.7.15 (60)` 最小功能闭环。

发布目标仍是四端同时交付。当前已完成“升版后复测 → 四包构建与身份校验”；后续固定顺序为“提交 B → 创建指向 B 的 annotated tag → Finalize → 备份/迁移/回读 → 原子激活策略 → 公网复下载 → 推送 → 非覆盖归档”。Finalize 通过前，目标 tag 不算有效发布证据。
...
以上能力已有 2.7.15 本地 Debug 制品和自动化证据，但没有真实生产数据库迁移、真机、Finalize 或线上部署证明。下列 3.1 至 3.5 是已经上线的 2.7.14 历史能力，不应与 2.7.15 候选状态混写。
...
- 2026-08-11 22:07 release Build 完成：Gradle 226 项任务全部执行；四角色各 337 项测试、合计 1,348 项，均 0 失败、0 错误、0 跳过；四角色 Lint 与独立 APK 审计全部通过。
- Build 源码提交 A：`49f44235af61b02bba7b6c06f5fe084685f1f2fb`。
- `releases/2.7.15/release-manifest.json` 为 schema 4 pending manifest，`finalizationStatus=pending`、`releaseEvidenceCommit=null`；文件 SHA-256 为 `489B9EE2138D8215B5B640F9A5B76B6511313CEE3F00F6542CC337C57DE1E788`。

| 端 | 文件 | 字节 | SHA-256 |
| --- | --- | ---: | --- |
| 用户 | `yiyunying-user-v2.7.15-debug.apk` | 96,707,619 | `9BBA2824B51E9F23EEE1026A74C64E43BB7A98AE2019106831FF9E2B28BAD89A` |
| 管理员 | `yiyunying-admin-v2.7.15-debug.apk` | 32,306,003 | `56131E68FE90C207A54314CAA34A08B9D91EA117A5862761259535D3180C06B4` |
| 授权平台 | `yiyunying-authorized-platform-v2.7.15-debug.apk` | 32,306,007 | `ED138B2896D97F7D58A5344AF9BB67301001E5A7E0B477A04867934DA0508147` |
| 平台总控 | `yiyunying-platform-owner-v2.7.15-debug.apk` | 32,306,003 | `61E8FD8BEB2C9630AF5496F23FAE2A2D1C6E7E734A17D8AFAA5398E115F525D8` |

四包签名证书 SHA-256 相同：`10162EBB7147EA0823C281D9F86FEFF2A353984A41497F17E196E50614E9B76E`。这些事实只证明本地 Build；pending manifest 不是 Finalize、标签、部署或公网发布证据。
...
- 2.7.15 四个 Debug APK 与 pending manifest 已生成并校验，但尚未 Finalize；`releaseEvidenceCommit` 仍为空，不能将 pending manifest 当作最终发布清单。
- 本次待提交的构建元数据与文档构成证据提交 B；精确 B 由 Finalize 后 manifest 回读，不预写。目标 annotated tag 只有通过 Finalize 核验才有效。当前尚未 Finalize，也未执行真实数据库迁移、私有资源迁移、后端/下载站部署、四条更新策略激活、生产/公网回读、推送或 E 盘归档。
...
1. 在不改动源码提交 A 和四包的前提下提交本次构建元数据/文档形成证据提交 B；创建指向 B 的 annotated tag；最后运行 Finalize 绑定并核验 B、tag、manifest 和四包。Finalize 通过前不得宣称 tag 是有效发布证据。
...
4. 只有部署证据稳定且 finalized manifest、Git refs 回读一致后才能推送并执行 E 盘新目录归档；不得移动/重建目标 tag，也不得覆盖 2.7.14 标签、制品、策略或恢复点。
...
18. 2.7.15/code60 已完成本地 Build，但不要把它写成已 Finalize、已迁移、已部署、已打标签、已推送或已归档；每一项都必须有独立回读证据。
...
> 易运盈本地 2.7.15（60）已从源码提交 A `49f44235af61b02bba7b6c06f5fe084685f1f2fb` 完成四端 Debug Build：226/226 Gradle tasks、四端各 337 项/合计 1,348 项测试、四端 Lint 和独立 APK 审计全部通过，四包与 schema 4 pending manifest 已校验。本次待提交的构建元数据/文档构成证据提交 B，精确 B 以 Finalize 后 manifest 为准；目标 annotated tag 只有 Finalize 核验后有效。当前尚未 Finalize、执行真实迁移/部署、推送或归档；线上仍为 2.7.14（59），四端 58→59 可更新、59→59 无更新。当前仍是 Debug/HTTP 路线，HTTPS 未配置，不能称为正式商业 Release。
