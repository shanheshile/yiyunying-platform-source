...
final class ActivityController
{
    ...
    public static function create(Request $request): \Yiyunying\Core\ApiResponse { return Response::success(['activity' => HierarchyActivityService::create(self::actor($request), $request->all())], '活动发布成功', 201); }
    ...
    public static function award(Request $request, array $params): \Yiyunying\Core\ApiResponse { $id = Validator::integer($request->input('submission_id'), 'submission_id', 1, PHP_INT_MAX); return Response::success(HierarchyActivityService::award(self::actor($request), (int) $params['activity_id'], $id), '悬赏结算成功'); }
    public static function close(Request $request, array $params): \Yiyunying\Core\ApiResponse { return Response::success(HierarchyActivityService::finish(self::actor($request), (int) $params['activity_id'], false), '活动已结束'); }
    public static function cancel(Request $request, array $params): \Yiyunying\Core\ApiResponse { return Response::success(HierarchyActivityService::finish(self::actor($request), (int) $params['activity_id'], true), '活动已取消'); }
    ...
    private static function actor(Request $request): array { return HierarchyActorService::admin(AuthService::admin($request)); }
}
