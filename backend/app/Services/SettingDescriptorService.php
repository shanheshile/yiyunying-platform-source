...
final class SettingDescriptorService
{
    ...
    private static function one(string $key, $value): array
    {
        $known = [
            ...
            'moment_post_audit' => ['动态发布需要审核', '开关', '开启后新动态先进入待审核列表；作者可查看自己的待审内容，其他用户仅能看到已通过动态。'],
            'moment_comment_audit' => ['动态评论需要审核', '开关', '开启后新评论先进入待审核列表；只有审核通过的评论会对其他用户展示并计入数量。'],
            ...
            'store_user_submit_enabled' => ['允许用户投稿应用', '开关', '控制当前应用的普通用户能否向应用商店投稿安装包；关闭后客户端入口会禁用，后端也会强制拒绝提交。'],
            'store_submit_audit' => ['应用投稿需要审核', '开关', '开启后用户投稿的应用必须由管理员审核通过后才在应用商店公开。'],
            ...
            'forum_unlock_max_price_balance' => ['帖子解锁最高价格', '余额/内容节', '限制整帖或单个内容节允许设置的最高余额价格；保存后同时约束新建、展示购买按钮和实际购买。'],
            'forum_unlock_max_future_days' => ['定时解锁最远日期', '天', '限制帖子正文、内容节和附件允许选择的最远自动解锁日期，避免异常或无法管理的超长期规则。'],
            'forum_paid_section_max_count' => ['单帖内容节上限', '节/帖子', '限制一个帖子可以创建的公开、付费或定时内容节总数；并发创建也会执行该上限。'],
            ...
            'media_cache_max_bytes' => ['媒体本地缓存上限', '字节/设备', '兼容旧客户端的媒体缓存建议总空间。'],
            'auto_download_cache_enabled' => ['允许自动下载缓存', '开关', '关闭后用户仍可手动下载，但客户端不会自动缓存聊天记录、资料或媒体。'],
            'auto_cache_allowed_categories' => ['允许自动缓存的类别', '多选', '可分别限制聊天记录、资料、图片、视频、语音、音频、文档、文件和表情包。'],
            'auto_cache_default_max_bytes' => ['自动缓存默认容量', '字节/设备', '新设备首次启用自动缓存时使用的建议空间。'],
            'auto_cache_max_bytes_limit' => ['自动缓存容量上限', '字节/设备', '用户可配置的缓存空间不得超过此管理员上限。'],
            'auto_cache_retention_days' => ['自动缓存最长保留期', '天', '自动缓存超过此时间后可被清理；受保护和手动下载内容不自动删除。0 表示不按时间限制。'],
            'auto_cache_network' => ['自动缓存允许网络', '选项', 'wifi 表示仅 Wi-Fi，wifi_mobile 表示 Wi-Fi 与移动网络，never 表示禁止自动缓存。'],
            'auto_cache_force_wifi_only' => ['强制仅 Wi-Fi 自动缓存', '开关', '开启后用户不能允许移动网络自动缓存。'],
            'auto_cache_policy_version' => ['自动缓存策略版本', '文本', '策略变更标识，客户端用于显示当前生效的管理规则版本。'],
            'video_autoplay_enabled' => ['允许视频自动播放', '开关', '控制当前应用是否允许客户端自动播放视频；用户仍可手动播放。'],
            'video_autoplay_network' => ['视频自动播放网络上限', '选项', '管理员允许的最高网络范围：wifi、wifi_mobile 或 never。'],
            'video_autoplay_default_network' => ['视频自动播放默认网络', '选项', '新设备默认使用的自动播放网络范围，用户可在管理员允许范围内调整。'],
