...
- Android 本地 Debug 候选：`2.7.15`（`versionCode 60`）；四包已从源码提交 A `49f44235af61b02bba7b6c06f5fe084685f1f2fb` 完成 Build 并通过独立 APK 审计。本次构建元数据/文档提交构成证据提交 B，精确 B 以 finalized manifest 回读为准；目标 annotated tag 只有 Finalize 核验后才有效。当前尚未 Finalize、部署、推送或归档
...
- “源码入口存在”不等于“生产环境已验收”。
- 本地 2.7.15 四包 Build 已完成，但 pending manifest 不等于 Finalize、迁移、部署、标签、推送或归档已经完成；每项必须有独立证据。
